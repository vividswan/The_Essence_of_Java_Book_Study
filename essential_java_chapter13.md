...
- 멀티 코어를 잘 활용할 수 있는 멀티 쓰레드 프로그래밍이 중요해짐
  - JDK 1.7부터 fork & join 프레임웍을 추가
  - 하나의 작업을 작은 단위로 나눠서 여러 쓰레드에서 동시에 처리하는 것을 쉽게 만들어줌
- 두 클래스 중 하나를 상속받아서 구현
  - RecursiveAction : 반환값이 없는 작업 구현
  - RecursiveTask : 반환값이 있는 작업을 구현
- compute() 추상 메서드를 구현해야 함
  - run()과 start()의 관계처럼 invoke()로 호출을 해야 작업을 실행
- fork & join 프레임웍에서 제공하는 ForkJoinPool이라는 쓰레드풀에서 쓰레드를 사용
  - 지정된 수의 쓰레드를 미리 생성한 뒤 반복해서 재사용 가능
  - 반복 생성 억제, 적당한 쓰레드 생성 (성능을 저하시킬 정도로 쓰레드를 만들지 않음)
  - 각 쓰레드는 자신의 작업 큐에 담긴 작업 순서대로 처리

다른 쓰레드의 작업 훔쳐 오기

- 자신의 작업 큐가 비어있는 쓰레드는 다른 쓰레드의 작업 큐에서 작업을 가져와서 수행
- 쓰레드풀에 의해 자동적으로 이루어짐
- 여러 쓰레드가 골고루 작업을 나누어 처리

fork()와 join()

- fork() : 해당 작업을 쓰레드 풀의 작업 큐에 넣는 비동기 메서드
  - 비동기 메서드는 다른 쓰레드에게 작업을 수행하도록 지시만 하고 결과를 기다리지 않음
- join() : 해당 작업의 수행이 끝날 때까지 기다렸다가 수행이 끝나면 결과를 반환하는 동기 메서드
